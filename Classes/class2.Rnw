...
<<results='asis'>>=
print(xtablm1,floating=FALSE,size='footnotesize')
@
...
\item Now, we have been adjusting but really didn't know (without those
  figures) if the intervention and control neighborhoods really differ enough
  on those variables to make us worries that these variables might confuse or
  confound our interpretations. How different should the intervention and
  control groups be such that we begin to worry that we should engage with
  the alternative explanations represented by those variables? How should we
  measure "difference"? In theory we are talking about differences in
  multivariate distribution, but could we get away with looking at other kinds of
  differences? What standard should we use to assess these differences? (Often
  called "balance")
  
  One idea, proposed by \citet{hansen:bowers:2008}, is to use the randomized
  experiment as a standard. They propose to test whether the intervention
  groups differ in terms of their means beyond that which we would expect from
  a randomized experiment. Because  our hypothesis refers to an experiment, we
...
The object created by xBalance has a plot method. Here we plot the
standardized differences in order of the one-by-one balance test $p$-values.

<<out.width='.5\\textwidth',tidy=FALSE>>=
plot(xb1, which.vars=names(xb1$results[order(xb1$results[,"p","raw"]),"p",]))
@
...
Below we can see the call to the \texttt{fullmatching} function and also see
some of the default output. How do you think you should interpret this output?
...
\item The \texttt{optmatch} package produces optimal full matched designs. The
 ...
 %% Insert a demo of greedy versus optimal and with versus without
 %replacement.  Perhaps see about another kind of optimal? 
