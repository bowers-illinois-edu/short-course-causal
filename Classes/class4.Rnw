...
\begin{enumerate}
  ...
  \item  We'll continue to work with the Cerd\'{a} et al data. We completed
    ...
    estimation are unbiased). We even confronted some of the deeper questions
    about choice of covariates for adjustment and M-bias.
...
\item Let's start with your favorite matched design from last time.

  Below I play around a bit with an alternate (and easier) penalized
  propensity score model, and also show some of the other options to
  \texttt{fullmatch}. 
...
## Propensity score using ridge regression with tuning parameter chosen via 
## cross-validation (10-fold) with correct classification as the criteria
...
mahalDist<-match_on(balfmla,data=meddatRanks)
...
@
What do these effect estimates mean?

\item Now, let's review statistical inference: If we tested a hypothesis about
  one of these effects, what would it mean?  (i.e. Imagine that we said, $H_0:
  \tau=0$ and got $p=.02$, how would you explain this to an educated person
  who is not a statistician? What about $p=.8$?) If we produced a confidence
  interval, and it did not contain 0 what would it mean? If we produced a
  confidence intervals and it did contain 0, what would it mean?
...
\item  So, now we have some sense about why people like the ATE (1) it
  ...
  \citet{linlr:2011}   suggests that the "iid" (independent and identically
...
<<>>=
## First using the Dunning derivation also discussed in
...
meddat$HomChange<-with(meddat,HomRate08-HomRate03)

specpars.ps(dat=meddat,covs=adjcovs,dropcov="nhPopD",
	    type=1, outcome="HomChange", treatment="nhTrt",
...
specpars.ps(dat=meddat,covs=adjcovs,dropcov="nhClass",
	    type=2, outcome="HomChange", treatment="nhTrt",
	    fitfunction=brglm,thecaliper=3,
	    fmcontrol= list(tol=.00001,min.controls=.25,max.controls=4))
...
specps.res<-sapply(adjcovs,function(thecov){
		   ...
		   specpars.ps(dat=meddat,covs=adjcovs,dropcov=thecov,
			       ...
			       outcome="HomChange", treatment="nhTrt",
			       fitfunction=brglm,thecaliper=3,
			       fmcontrol= list(tol=.00001,min.controls=.25,max.controls=4))
	    })
...
\begin{comment}
  ...
  So, let us consider the  baseline outcome: The results are, $\pcor$,
  $\teeW$, the treatment effect (i.e. coef on nhTrt) when this variable is
  \emph{not} included in the propensity model/matching; the finite-sample
  (HC2approx) standard error on this coef; the df for that regression (mainly
  relevant for factor variables); and the coef for nhTrt when this term is
  included in the matching/fixed-effects and corresponding standard error.
...
\begin{comment}
  ...
  So, we might read the that table as telling about the sensitivity of
  our treatment effect estimation to excluded variables with characteristics
  like those of the included variables.

  Here is a graphical version of that table:
  
<<plotcis,fig.width=8,fig.height=8,out.width='.5\\textwidth',fig.keep='last'>>=
