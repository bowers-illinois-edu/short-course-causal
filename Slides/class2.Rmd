...
test1<-independence_test(social.capital01~vm.community.norm2|nbp1,
			 data=wrkdat[!is.na(wrkdat$nbp1),], teststat="quadratic")
test2<-xBalance(vm.community.norm2~social.capital01,
		strata=list(nbp1=~nbp1), data=wrkdat, 
...
wrkdat[order(wrkdat$nbp1),c("social.capital01","scRank","scMD","vm.community.norm2","vmcn2MD","vmCRank","nbp1")][1:4,]
...
theHC2ci<-coefci(lm1,vcov=vcovHC(lm1,type="HC2"))
coeftest(lm1,vcov.=vcovHC(lm1,type="HC2"))
lm2 <- lm_robust(scMD~vmcn2MD,data=wrkdat[!is.na(wrkdat$nbp1),])
lm2
lm3 <- lm_robust(social.capital01~vm.community.norm2,fixed_effects=~nbp1,data=wrkdat)
lm3
table(wrkdat$vmCRank)
lm4 <- lm_robust(social.capital01~I(vmCRank-1),fixed_effects=~nbp1,data=wrkdat)
lm4
```
