...
Deciding to stratify rather than remove linear relationships raised new
   questions about:
   1. distance matrices (on single covariates, Mahalanobis
   distances, propensity score distances),
      - And in turn raised questions about the
   separation problem in logistic regression (and why we prefer `bayesglm` to
   `glm` when we create propensity scores).
   2. calipers,
   3. ratios of treated to control units,
   4. exact matching.

We did not talk about missing data on covariates (and the `fill.NAs` command in optmatch).
