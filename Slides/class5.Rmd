...
## So far:
 ...
 1. Introduced non-bipartite matching (see also *Design of Observational
    Studies* on Risk Set Matching and Longitudinal Applications)
 2. New questions arose:
    1. How to encourage pairs to **differ** in the causal/explanatory variable.
       (use the `far` option in `nmatch`).
    2. What does "balance" mean in this case? Does it compare to an experiment
       in some way? Which experiment?
    3. How to think about effects: should it just be higher-vs-lower? Or
       more-versus-less?
...
## Today: Sensitivity Analysis
 ...
 1. Agenda: Reasoning about how departures from our "as-if randomized" approach
    ...
    style and maybe demonstrating a more regression oriented style.
...
DiD approaches: to the extent that we claim to be estimating an ATE, we are
assuming "as-if randomized" rather than parallel trends.)
...
pm1 <- pairmatch(glm1,data=meddat)
meddat$pairm <- factor(pm1)
...
lm1
## Or using a permutation based test: since the focus in Rosenbaum is on p-values
meddat$nhTrtF <- factor(meddat$nhTrt)
set.seed(12345)
test1 <- oneway_test(HomRate0803~nhTrtF|pairm,data=meddat,distribution=approximate(nresample=10000))
pvalue(test1)[1]
test1a <- oneway_test(HomRate0803~nhTrtF|pairm,data=meddat,distribution=approximate(nresample=10000))
pvalue(test1a)[1]
```
...
@cinellihazlett2020 and @hosman2010 expresse sensitivity analysis in terms more familiar to people who are using linear regression for estimation and testing of the weak null. Here is a version using the `sensemakr` package:

```{r echo=TRUE}
library(sensemakr)
res1_lm <- lm(HomRate0803~nhTrt+pairm, data = meddat)
...
## you can do this:
sens2 <- sensemakr(estimate = coef(lm1)[["nhTrt"]], se = lm1$std.error[["nhTrt"]], treatment = "nhTrt", dof = lm1$df.residual)
...
## Summary and Questions:
 ...
 - Formal sensitivity analysis allows us to use the as-if randomized approach to statistical inference **provisionally** (versus fearfully): we can directly ask how our distributions would change under different forms of unobserved confounding.
 - The fact that a $p<.01$ would change to $p=.051$ if $\Gamma=3$ (or if $\Lambda$=4.5 and $\Delta$=14.7) does not mean that such an unobserved covariate exists.
 - Since we are reasoning about unobserved covariates, we have a naming problem: We have to ask: What covariate could conceivably play this role?
 - See the Rabb et al (2022) piece for an example of this kind of sensitivity analysis in application.
 - See Hazlett (2020) for an application of the regression based approach: "Angry or Weary?
How Violence Impacts Attitudes toward Peace among Darfurian Refugees".
