...
psmod2 <- glm(balfmla,data=meddat)
psdist2 <- match_on(psmod2,data=meddat)

mhdist2 <- match_on(balfmla,data=meddat,method = "rank_mahalanobis")

fmMh2 <- fullmatch(mhdist2,data=meddat)
summary(fmMh2,min.controls=0,max.controls=Inf)

fmPs2 <- fullmatch(psdist2,data=meddat)
summary(fmPs2,min.controls=0,max.controls=Inf)

xb5 <- balanceTest(update(balfmla,.~.+strata(fmPs2)+strata(fmMh2)+strata(fmMh)+strata(fmPs)),
                   data=meddat,report="all",p.adjust.method="none")
xb5$overall[,]
...
Without blocks, and with randomization, we estimate $\bar{\tau}$ with $\hat{\bar{\tau}}=\hat{\bar{y}}_1 - \hat{\bar{y}}_0$. And with $m$ treated units, $\hat{\bar{y}}_1  = (1/m) \sum_{i \in Z_i=1} Y_i$  and  $\hat{\bar{y}}_0  = (1/(n-m)) \sum_{i \in Z_i=0} Y_i$
